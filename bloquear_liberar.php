...
//		$v_tipo = "2,3";
	...
	$arq_miolo_template = "template/bloquear_liberar.inc";
	...
	$arq_miolo_template = "template/alt_sit.inc";
	...
	foreach ($arr_bd_sqlserver as $chave=>$valor) {
		...
		if ($_POST["bloq_lib"] == "lib") {
			...
			if ($res_cartoes[SITUACAO][0] != "Liberado") {
				...
				$sql = "update pessoas
						set pessit_numero = (select top 1 pessit_numero from pessoa_situacao where pessit_descricao = 'Liberada' order by 1 desc)
						where pes_numero = ".$_POST["pes_numero"];
	...
	header('Location: bloquear_liberar.php?acao=pesquisando&f_tipo='.$f_tipo.'&f_RA='.$f_RA.'&f_nome='.$f_nome);
	exit;
